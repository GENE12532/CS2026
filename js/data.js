...
这些新方法显著提升了分布式隐私保护技术的实用性，为在分布式环境下实现高效用、安全的数据利用提供了新的解决思路，并为未来的隐私计算与可信人工智能奠定了更加坚实的理论与应用基础。`, 
    ...
    },
    {
      ...
      title: "Secure GPT：GPT模型的隐私保护安全多方推理",
      abstract: `GPT是一种先进的自然语言处理模型，擅长理解和生成自然语言。随着GPT模型的广泛应用，越来越多基于预训练生成模型的云推理服务应运而生。当用户将数据上传至云服务器以使用这些服务时，如何保障数据的隐私和安全成为了一项重要挑战。我们提出了一种针对GPT的多方隐私保护Transformer推理框架SecureGPT，并为GPT的Transformer层设计了多方隐私保护计算框架，并在半诚实安全模型下证明了框架的安全性。`, 
    ...
    },
    {
      ...
      image: "img/ym.jpg",
      title: "开源代码漏洞治理",
      abstract: `开源软件供应链的漏洞治理有两个重要环节，首先是面对软件供应链链条中环节多、资产理不清的问题，漏洞的治理需具备供应链全链路的精准漏洞感知能力。另一方面，在漏洞的披露与发现后，想要通过有限的漏洞信息构建对漏洞的全景认知，并完成追踪和修复，还需要有可靠的漏洞管理、处置能力。报告将围绕漏洞被定位后如何进行高效的评估和处置两点，从漏洞影响范围、漏洞危害性、漏洞补丁等关键技术维度介绍漏洞信息增强方法，并依托于高质量的漏洞数据介绍如何开展可靠的漏洞补丁部署和漏洞修复评估。`, 
    ...
    },
    {
      ...
      image: "img/zjd.jpg",
      title: "赤兔：大模型推理系统",
      abstract: `赤兔（Chitu）是由清华大学高性能计算研究所与清华系科创企业清程极智联合开发的开源大模型推理引擎。2025年3月14日首次开源，并于同年8月1日发布v0.4版本。该引擎定位于人工智能推理框架，支持在非英伟达Hopper架构GPU及昇腾、沐曦等国产芯片上原生运行FP8精度模型。赤兔通过底层算子优化（如GeMM、MoE指令级重构）和编译技术创新，首次实现在无FP8硬件单元的算力芯片上直接部署FP8模型，在昇腾910B单机推理时Qwen3-32B模型吞吐量达5000token/s 。其部署DeepSeek-R1-671B满血版大模型时，相比部分国外框架可实现GPU使用量减少50%且推理速度提升3.15倍。v0.4版本新增数据并行结合专家并行策略，代码与镜像已在魔乐社区等国内平台开源。`, 
    ...
    },
    {
      ...
      title: "可信可解释人工智能",
      abstract: `大部分计算机视觉算法的输入为连续或者稠密的图像数据，输出为人类可以理解的类别、位置、深度等离散或者稀疏语义空间。由于定义域和值域集合的基数不一致，这些计算机视觉算法不满足well-posed问题的第三个条件，是经典的ill-posed问题。纯数据驱动的计算机视觉算法是否既能具有对烟雨雾霾等复杂扰动的鲁棒性？又能避免的dedicated对抗扰动或者后门投毒? 可解释性与准确率是否不可兼得？是否存在裨益计算机视觉任务的正向扰动？我们团队正在探索的这些问题的答案，本报告将详细我们在该领域的最新进展。`, 
    ...
    },
    {
      ...
      abstract: `针对大数据时代，信息碎片化、隐私差异化等特点，解决“抗大数据分析的隐私保护” 这一科学问题，围绕抗关联分析的大数据发布隐私保护、大数据隐私保护技术的评估方法等主要研究内容，研究大数据隐私感知和标记、抗关联分析大数据发布、动态增量数据发布的隐私保护、隐私保护机制的效果评估等方面问题。目标是解决数据发布如何抵抗能力不断增长的大数据关联分析，实现大数据增量发布的隐私保护，研究成果将基于医疗大数据和社交网络大数据进行验证示范。`, 
  ...
  committees: {
    generalChairs: {
      title: "大会主席",
      members: [
        { name: "张方国", university: "中山大学", image: "img/zfg.jpg" },
      ],
    },
    programChairs: {
      title: "程序委员会主席",
